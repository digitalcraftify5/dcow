...
export const InstallPwaPrompt: React.FC = () => {
  ...
  React.useEffect(() => {
    // 1. Check if user already dismissed or installed app in this session
    ...
    // 4. Capture native browser beforeinstallprompt event (Android, Chrome, Edge)
    const handleBeforeInstallPrompt = (e: Event) => {
      e.preventDefault();
      setDeferredPrompt(e as BeforeInstallPromptEvent);
      // Delayed display after 3s for unobtrusive arrival
      setTimeout(() => setShowPrompt(true), 3000);
    };

    window.addEventListener("beforeinstallprompt", handleBeforeInstallPrompt);

    // If iOS or standard browser without prompt event, display fallback floating prompt after 4s
  ...
  const handleInstallClick = async () => {
    if (deferredPrompt) {
      ...
      // General Desktop / Fallback Prompt
      ...
      <AnimatePresence>
        {showPrompt && (
          <motion.div
            ...
            className="fixed bottom-5 right-4 sm:right-6 z-50 max-w-sm sm:max-w-md w-[calc(100%-2rem)] p-4 sm:p-5 rounded-3xl glass-card border border-cyan-500/50 bg-zinc-950/95 shadow-[0_0_40px_rgba(0,240,255,0.3)] text-white space-y-4"
          >
            {/* Top Bar with Dismiss */}
            <div className="flex items-center justify-between border-b border-zinc-800/80 pb-3">
              <div className="flex items-center gap-2.5">
                <div className="relative h-14 w-14 rounded-2xl overflow-hidden bg-zinc-900 border border-cyan-500/50 p-1 shadow-[0_0_15px_rgba(0,240,255,0.4)]">
                  <Image
                    src="/logo.png"
                    alt="Digital Craftify Web App"
                    width={56}
                    height={56}
            ...
            <p className="text-xs text-zinc-300 leading-relaxed font-sans">
              Install the official <strong>Digital Craftify Web App</strong> for 1-tap launch, offline support &amp; direct executive connect with Tanveer Hussain.
            </p>
            ...
            >
              ...
              <div className="space-y-4 text-xs font-mono text-zinc-300">
                <div className="p-4 rounded-2xl bg-zinc-900 border border-zinc-800 space-y-2 text-center">
                  <span className="text-xs text-zinc-400 uppercase tracking-widest block font-bold">KEYBOARD SHORTCUT</span>
